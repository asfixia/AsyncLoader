...
(function (AsyncLoader) {
    ...
    function loadScriptOnce(src, callback) {
        var srcArr = isArray(src) ? src : Array(src);
        loadScriptsOnce(srcArr, callback);
    }
    AsyncLoader.loadScriptOnce = loadScriptOnce;
    /**
     ...
     */
    function loadScriptsOnce(srcArr, callback) {
        srcArr = srcArr.sort();
        filterLoaded(srcArr);
        if (srcArr.length) {
     ...
     */
    function wasLoaded(src) {
        _loadscript[src] = 1 /* COMPLETED */;
     ...
     */
    function filterLoaded(srcArr) {
        for (var i = srcArr.length - 1; i >= 0; i--)
            if (_loadscript[srcArr[i]] && _loadscript[srcArr[i]] === 1 /* COMPLETED */)
                srcArr.splice(i, 1);
    }
    /**
     ...
     */
    function addLoadCallback(loadingSrcs, callback) {
        var wasFound = false;
